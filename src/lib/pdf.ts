...
export async function generatePDF(data: PDFData) {
  ...
  doc.setProperties({
  title: "QuoteMate AI",
  subject: "Professional trade quote",
  author: "QuoteMate AI",
  creator: "QuoteMate AI",
});
  ...
  try {
    ...
    doc.addImage(img, "PNG", 20, 10, 45, 20);
  } catch (error) {
    console.log("Logo failed:", error);
  }
}
  ...
  doc.setFontSize(20);
  doc.setFont("helvetica", "bold");
  doc.text(data.businessName || "QuoteMate AI", 75, 20);
...
addText(`Date: ${data.quoteDate}`);
  ...
  doc.text(
    "Generated by QuoteMate AI | ABN: 15 154 226 673 | hello@quotemateai.com.au",
    20,
    y
  );
